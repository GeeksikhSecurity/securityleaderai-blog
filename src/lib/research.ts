...
// ⚠️ HARDCODED DATES: every `date:` in researchArticles below is hand-maintained
// here as ISO YYYY-MM-DD — it is NOT read from markdown frontmatter. Grep
// `HARDCODED-DATE` to audit. When an item maps to a blog post, keep its date in
// sync with that post's frontmatter; otherwise use the git-history add date.
